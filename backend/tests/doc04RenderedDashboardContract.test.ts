...
assert.ok(legacyScript.includes('path.indexOf("/dashboard") === 0'), "DOC-04 legacy renderer must treat local /dashboard as the document preview so sidebar navigation cannot fall back to the empty state.");
...
assert.ok(script.includes('path.indexOf("/dashboard") === 0'), "DOC-04 commercial renderer must treat local /dashboard as the document preview so clicking the sidebar Data Overview keeps the commercial dashboard.");
